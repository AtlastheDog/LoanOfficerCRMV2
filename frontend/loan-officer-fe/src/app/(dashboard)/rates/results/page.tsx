...
export default function RateSheetResults() {
  ...
  const handleSaveRates = async () => {
    setIsSaving(true)
    try {
      await api.post("/api/rate_points", { rate_points: ratePoints, scenario: scenarioData })
  ...
  const handleAnalyzeLeads = async () => {
    ...
    try {
      await api.post("/api/rate_points", { rate_points: ratePoints, scenario: scenarioData })
      router.push("/dashboard/leads/analyze")
    } catch (err) {
  ...
  return (
    <div className="container mx-auto px-4">
      <div className="mb-6 flex items-center">
        <Link href="/dashboard/rates/upload" className="mr-4 text-blue-600 hover:text-blue-800">
